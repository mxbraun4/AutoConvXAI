...
class AutoGenEvaluator:
    ...
    def evaluate_entities_match(self, expected: Dict[str, Any], actual: Dict[str, Any]) -> bool:
        ...
        actual_entities = {}
        if 'command_structure' in actual:
            actual_entities = actual['command_structure']  # Final validated entities (priority)
        elif 'action_response' in actual and 'entities' in actual['action_response']:
            actual_entities = actual['action_response']['entities']  # Fallback to original entities
        elif 'entities' in actual:
            actual_entities = actual['entities']
        
        # IMPROVED: Flexible handling of empty expected entities
        # If expected entities is empty {}, accept both {} and {field: null} formats
        if not expected_entities:
            # Handle both empty object {} and null-filled structure
            if not actual_entities:
                # actual_entities is also {} - perfect match
                return True
    ...
    def evaluate_single_case_initial_only(self, test_case: Dict[str, Any]) -> EvaluationResult:
        """Evaluate a single test case using ONLY the initial extraction (first agent output)."""
            ...
            try:
                ...
                self.logger.warning(f"=== INITIAL EXTRACTION DEBUG ===")
                self.logger.warning(f"Has _last_revision_info: {hasattr(self.decoder, '_last_revision_info')}")
                self.logger.warning(f"Initial extraction is None: {initial_extraction is None}")
                if initial_extraction:
                    self.logger.warning(f"Initial extraction: {initial_extraction}")
                else:
                    self.logger.warning(f"FALLING BACK TO FINAL OUTPUT")
                    self.logger.warning(f"Final output: {autogen_output}")
                self.logger.warning(f"=== END DEBUG ===")
                
                # Use initial extraction for evaluation if available, otherwise fall back to final output
                evaluation_target = initial_extraction if initial_extraction else autogen_output
                
                # Check matches using initial extraction
                action_match = self.evaluate_action_match(expected_json, evaluation_target)
                entities_match = self.evaluate_entities_match(expected_json, evaluation_target)
                overall_match = action_match and entities_match
                
                return EvaluationResult(
                    question=question,
                    expected_json=expected_json,
                    autogen_output=evaluation_target,  # Store what we actually evaluated
                    action_match=action_match,
                    entities_match=entities_match,
                    overall_match=overall_match,
                    initial_extraction=initial_extraction
    ...
    def evaluate_parallel_initial_only(self, test_cases: List[Dict[str, Any]], max_workers: int = 1, batch_refresh: bool = True) -> List[EvaluationResult]:
            ...
            for i, test_case in enumerate(test_cases):
                result = self.evaluate_single_case_initial_only(test_case)
                results.append(result)
                completed += 1
                
                # Clean progress logging
                status = "✓" if result.overall_match else "✗"
                self.logger.info(f"Case {completed}/{total}: {status} (Initial Only)")
    ...
    def generate_report(self, results: List[EvaluationResult]) -> Dict[str, Any]:
        ...
        # DETAILED BREAKDOWN: Four categories of results
        both_correct = sum(1 for r in results if r.action_match and r.entities_match)
        action_correct_entity_wrong = sum(1 for r in results if r.action_match and not r.entities_match)
        action_wrong_entity_correct = sum(1 for r in results if not r.action_match and r.entities_match)
        both_wrong = sum(1 for r in results if not r.action_match and not r.entities_match)
        
        # Validation tracking removed for cleaner evaluation
        ...
        report = {
            ...
            # DETAILED BREAKDOWN: Four categories
            'both_correct': both_correct,
            'both_correct_rate': both_correct / total_cases if total_cases > 0 else 0,
            'action_correct_entity_wrong': action_correct_entity_wrong,
            'action_correct_entity_wrong_rate': action_correct_entity_wrong / total_cases if total_cases > 0 else 0,
            'action_wrong_entity_correct': action_wrong_entity_correct,
            'action_wrong_entity_correct_rate': action_wrong_entity_correct / total_cases if total_cases > 0 else 0,
            'both_wrong': both_wrong,
            'both_wrong_rate': both_wrong / total_cases if total_cases > 0 else 0,
...
def evaluate_all_cases():
    ...
    print(f"  Perfect Cases: {report['both_correct']} ({report['both_correct_rate']:.2%})")
    print(f"  Action-Only Errors: {report['action_correct_entity_wrong']} ({report['action_correct_entity_wrong_rate']:.2%})")
    print(f"  Entity-Only Errors: {report['action_wrong_entity_correct']} ({report['action_wrong_entity_correct_rate']:.2%})")
    print(f"  Complete Failures: {report['both_wrong']} ({report['both_wrong_rate']:.2%})")
