...
"""
        ...
        try:
            ...
            if dataset:
                ...
                context_components.append(
                    f"AVAILABLE FEATURES: {', '.join(all_features)}"
                )

                # Target class information for prediction context
                if hasattr(conversation, "class_names"):
                    context_components.append(
                        f"TARGET CLASSES: {', '.join(map(str, conversation.class_names))}"
                    )

        except Exception as e:
            # Graceful degradation when context extraction fails
            context_components.append(
                "DATASET CONTEXT: Context extraction failed - proceeding with limited information"
            )
        ...
        try:
            if hasattr(conversation, "temp_dataset") and conversation.temp_dataset:
                ...
                if temp_size < full_size:
                    ...
                    context_components.append(
                        f"DATASET STATUS: Currently filtered to {temp_size} out of {full_size} records ({filter_percentage:.1f}%)"
                    )

                    # Add details about the filtering if available
                    if (
                        hasattr(conversation, "parse_operation")
                        and conversation.parse_operation
                    ):
                    ...
                    if (
                        hasattr(conversation, "last_parse_string")
                        and conversation.last_parse_string
                    ):
                        last_parse = (
                            conversation.last_parse_string[-1]
                            if conversation.last_parse_string
                            else ""
                        )
                        if last_parse:
                            context_components.append(
                                f"PREVIOUS QUERY ACTION: {last_parse}"
                            )

                    # Add guidance for agents
                    context_components.append(
                        "DISCUSSION FOCUS: Consider whether this new query needs:"
                    )
                    context_components.append("- Full dataset analysis (reset filter)")
                    context_components.append(
                        "- Filtered dataset analysis (keep current filter)"
                    )
                    context_components.append(
                        "- New filtering criteria (apply new filter)"
                    )
                else:
                    context_components.append(
                        f"DATASET STATUS: Using full dataset ({full_size} records)"
                    )
            else:
                # No filtering context available
                context_components.append(
                    "DATASET STATUS: Full dataset (no filtering applied)"
                )
        except Exception as e:
        ...
        try:
            ...
            if dataset and model and hasattr(dataset, "contents"):
                ...
                if y_true and X:
                    ...
                    context_components.append(
                        f"- Ground truth: {ground_truth_positive} positive cases ({gt_percentage:.1f}%)"
                    )
                    context_components.append(
                        f"- Model predictions: {predicted_positive} positive cases ({pred_percentage:.1f}%)"
                    )

                    # Add follow-up detection hints
                    if predicted_positive < ground_truth_positive:
                        context_components.append(
                            "- Model underpredicts (conservative)"
                        )
                    elif predicted_positive > ground_truth_positive:
                        context_components.append("- Model overpredicts (aggressive)")
                    else:
                        context_components.append(
                            "- Model predictions match ground truth"
                        )

                    context_components.append(
                        "NOTE: If user references these results or asks analytical questions about them, consider 'followup' intent"
                    )
    ...
    async def complete(
        self, user_query: str, conversation, grammar: str = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(collaboration_result, "messages"):
                ...
                for i, m in enumerate(collaboration_result.messages):
                    ...
                    logger.warning(
                        f"Message {i}: source={source} content_length={content_len}"
                    )
            else:
            ...
            logger.info(
                f"Stage 3: Agent collaboration completed with {len(collaboration_result.messages) if hasattr(collaboration_result, 'messages') else 0} messages"
            )
            logger.info(f"Collaboration result type: {type(collaboration_result)}")
            if hasattr(collaboration_result, "messages"):
                logger.info(
                    f"Messages attribute exists, length: {len(collaboration_result.messages)}"
                )
                for i, msg in enumerate(collaboration_result.messages):
                    logger.info(
                        f"Message {i}: source={getattr(msg, 'source', 'unknown')}, content_length={len(getattr(msg, 'content', ''))}"
                    )
            else:
        ...
        if (
            hasattr(collaboration_result, "messages")
            and len(collaboration_result.messages) > 0
        ):
    ...
    def _analyze_revisions(self, messages) -> Dict[str, Any]:
        ...
        if len(extractor_responses) >= 2:
            ...
            if len(validator_responses) > 0 and (
                revision_info["action_changes"] or revision_info["entity_changes"]
            ):
    ...
    def _update_revision_stats(self, revision_info: Dict[str, Any]):
        ...
        logger.info(
            f"REVISION TRACKER: Total queries={self.revision_stats['total_queries']}, "
    ...
    def _configure_termination_condition(self):
        ...
        logger.info(
            f"Using MaxMessageTermination with min_rounds={min_rounds} to ensure validator participation"
        )
    ...
    def _create_agent_team(self, termination_handler):
        ...
        if termination_handler:
            ...
            if "termination_condition" in signature.parameters:
                ...
                logger.info(
                    "Termination condition attached using 'termination_condition' parameter"
                )
            elif "termination" in signature.parameters:
                team_parameters["termination"] = termination_handler
                logger.info(
                    "Termination condition attached using 'termination' parameter"
                )
            elif "termination_checker" in signature.parameters:
                team_parameters["termination_checker"] = termination_handler
                logger.info(
                    "Termination condition attached using 'termination_checker' parameter"
                )
            else:
                logger.warning(
                    "Could not attach termination condition - no supported parameter found"
                )
    ...
    def _process_agent_responses(
        ...
        agent_messages = collaboration_result.messages[
            1:
        ]  # Skip the first message (user message)
        ...
        for message_index, message in enumerate(agent_messages):
            ...
            if hasattr(message, "content") and message.content:
                logger.warning(
                    f"=== AGENT MESSAGE {original_index} (chronological order) FULL CONTENT ==="
                )
                ...
                if "CONSENSUS_REACHED" in message.content:
                    logger.info(
                        f"Found CONSENSUS_REACHED marker in agent message {original_index} - consensus should have triggered termination"
                    )
            else:
        ...
        for message_index, message in enumerate(reversed(agent_messages)):
            ...
            original_index = (
                len(agent_messages) - message_index
            )  # This gives us 3, 2, 1 for agent messages
            ...
            if extracted_response and isinstance(extracted_response, dict):
                logger.info(
                    f"Agent message {original_index}: Successfully extracted JSON: {extracted_response}"
                )
                # Classify response by content structure
                response_type = self._classify_response_type(extracted_response)
                logger.info(
                    f"Agent message {original_index}: Classified as response type: {response_type}"
                )
                ...
                if response_type == "action":
                    if not intent_validation_response:
                        ...
                        logger.info(
                            f"Agent message {original_index}: Set as primary response (newest agent message)"
                        )
                        ...
                        logger.info(
                            f"Agent message {original_index}: Set as fallback response"
                        )
            else:
                logger.warning(
                    f"Agent message {original_index}: Failed to extract valid JSON response"
                )

        # Log what we found from agents for debugging
        logger.info(
            f"Agent responses found: Primary={bool(intent_validation_response)}, Fallback={bool(intent_response)}"
        )
    ...
    def _extract_json_response(self, message, message_index: int) -> Optional[Dict]:
        ...
        if not hasattr(message, "content") or not message.content:
            logger.debug(
                f"Message {message_index}: No content attribute or empty content"
            )
            return None
        ...
        try:
            ...
            if json_block_match:
                ...
                logger.debug(
                    f"Message {message_index}: Found JSON code block: {json_str[:100]}..."
                )
            ...
            if json_block_match:
                json_str = json_block_match.group(1).strip()
                logger.debug(
                    f"Message {message_index}: Found generic code block: {json_str[:100]}..."
                )
                result = json.loads(json_str)
                return result if isinstance(result, dict) else None
            ...
            if content.strip().startswith("{") and content.strip().endswith("}"):
                ...
                logger.debug(
                    f"Message {message_index}: Found direct JSON: {json_str[:100]}..."
                )
                result = json.loads(json_str)
                return result if isinstance(result, dict) else None
                    ...
                    if brace_count == 0 and start_pos != -1:
                        ...
                        try:
                            ...
                            if isinstance(result, dict):
                                logger.debug(
                                    f"Message {message_index}: Found embedded JSON: {json_candidate[:100]}..."
                                )
                                return result
        ...
        logger.warning(
            f"Message {message_index}: No valid JSON found after trying all patterns. Content preview: {content[:200]}..."
        )
        return None
    ...
    def _integrate_agent_outputs(
        self, intent_response, intent_validation_response
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        if intent_response and intent_validation_response:
            return self._create_complete_response(
                intent_response, intent_validation_response
            )
    ...
    def _create_complete_response(
        ...
        validated_action = intent_validation_response.get(
        ...
        return {
            ...
            "validation_response": {
                "requires_full_dataset": intent_validation_response.get(
                    "requires_full_dataset", False
                ),
                "critical_analysis": intent_validation_response.get(
                    "critical_analysis", ""
                ),
                "alternative_interpretations": intent_validation_response.get(
            ...
            "identified_issues": intent_validation_response.get(
                "alternative_interpretations", []
            ),
        }
    ...
    def _create_partial_response(self, intent_response) -> Dict[str, Any]:
        ...
        confidence_score = (
            intent_response.get("confidence", 0.8) * 0.9
    ...
    def _create_minimal_response(
        self, user_query: str, collaboration_result
    ) -> Dict[str, Any]:
        """
        ...
        agent_messages = (
            collaboration_result.messages[1:]
            if len(collaboration_result.messages) > 1
            else []
        )
        for i, message in enumerate(agent_messages):
            extracted_response = self._extract_json_response(
                message, i + 1
            )  # Use proper index for agent messages
            if (
                extracted_response
                and isinstance(extracted_response, dict)
                and "action" in extracted_response
            ):
    ...
    def _execute_in_thread(
        ...
        def run_in_isolated_thread():
            ...
            try:
                logger.info(
                    f"Starting async processing for query: {user_query[:50]}..."
                )
                result = new_loop.run_until_complete(
                    self.complete(user_query, conversation, grammar)
                )
                ...
                return self._create_error_response(
                    "Processing timeout - please try a simpler query"
                )
            except Exception as e:
                logger.error(f"Error in async processing: {e}")
                return self._create_error_response(
                    f"AutoGen processing error: {str(e)}"
                )
            finally:
                try:
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                ...
                result = future.result(
                    timeout=60
                )  # Increased to 60 seconds for reliable AutoGen processing
            ...
            return self._create_error_response(
                "System timeout after 60 seconds - agents need more focus"
            )
        except Exception as e:
            logger.error(f"Thread execution failed: {e}")
            return self._create_error_response(f"Thread execution error: {str(e)}")

    def _execute_with_new_loop(
        self, user_query: str, conversation, grammar: str = None
    ) -> Dict[str, Any]:
    ...
    async def cleanup(self, full_cleanup=True):
        ...
        try:
            ...
            logger.debug(
                f"AutoGen decoder resources cleaned up (full_cleanup={full_cleanup})"
            )
        except Exception as e:
